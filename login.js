// login.js — wires the UI you provided to Supabase auth and "Remember Me"
...
// Password strength scoring (same logic)
function score(p) {
  let s = 0;
  if (!p) return 0;
  if (p.length >= 6) s++;
...
function attachAuthHandlers() {
  ...
  async function register() {
    ...
    try {
      ...
      // Remember email if checkbox checked on register (use login remember checkbox state if present)
