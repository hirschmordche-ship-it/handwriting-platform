...
if (regPass) {
  regPass.addEventListener("input", e => {
    ...
    if (regBar) regBar.style.width = ["5%", "33%", "66%", "100%"][s];
    if (regBar) regBar.style.background = ["#eee", "#ffe066", "#ffd166", "#2d6a4f"][s];
    if (regText) regText.textContent = ["", "Weak", "Medium", "Strong"][s];
  });
}
